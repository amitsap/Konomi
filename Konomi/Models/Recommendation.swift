...
final class Recommendation {
    ...
    var identityKey: String {
        if let tmdbID {
            return "\(mediaTypeRaw)|tmdb|\(tmdbID)"
        }
        if let openLibraryID, !openLibraryID.isEmpty {
            return "\(mediaTypeRaw)|openlibrary|\(openLibraryID)"
        }
        return "\(mediaTypeRaw)|text|\(Self.normalizedIdentityPart(title))|\(Self.normalizedIdentityPart(creator))"
    }
