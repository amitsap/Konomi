...
struct ClaudeRequestOptions: Equatable, Sendable {
    let effort: String
    let maxTokens: Int

    static let tasteProfile = Self(effort: "high", maxTokens: 6_000)
    static let recommendations = Self(effort: "high", maxTokens: 16_000)
    static let serendipityExplanation = Self(effort: "low", maxTokens: 512)
    static let connectionValidation = Self(effort: "low", maxTokens: 128)
}
...
enum ClaudeService {
    ...
    static func sendWithSystem(_ system: String, user: String, options: ClaudeRequestOptions) async throws -> String {
    ...
    static func send(messages: [[String: Any]], system: String? = nil, options: ClaudeRequestOptions) async throws -> String {
        let body = requestBody(messages: messages, system: system, options: options)
        return try await send(body: body)
    }
    ...
    static func balancedJSONPrefix(from text: String) -> String? {
        var stack: [Character] = []
        var inString = false
        var isEscaped = false
