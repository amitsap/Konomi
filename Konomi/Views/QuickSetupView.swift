...
final class QuickSetupViewModel: ObservableObject {
    ...
    let tvFilters: [QuickSetupFilter] = [
        ...
        .init(id: "tv-scifi-fantasy", title: "Sci-Fi & Fantasy", kind: .genre(10765)),
...
struct QuickSetupView: View {
                ...
                ForEach(viewModel.currentFilters) { filter in
                    Button {
                        ...
                        Text(filter.title)
                            ...
                            .padding(.vertical, 8)
                            .background(viewModel.currentFilter.id == filter.id ? KonomiTheme.persimmon : KonomiTheme.inkSecondary.opacity(0.12))
                            .foregroundStyle(viewModel.currentFilter.id == filter.id ? KonomiTheme.onPersimmon : KonomiTheme.ink)
                            .clipShape(Capsule())
                    }
                    .buttonStyle(.plain)
                }
            ...
            VStack(spacing: 12) {
                Button {
                    ...
                    VStack(spacing: 4) {
                        ...
                        Text("Fastest option")
                            .font(.caption)
                            .foregroundStyle(KonomiTheme.onPersimmon.opacity(0.85))
                    }
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, 16)
                    .background(KonomiTheme.persimmon)
    ...
    private var ratingView: some View {
        ...
        return VStack(spacing: 20) {
            ...
            AsyncImage(url: URL(string: item.coverURLString ?? "")) { phase in
                ...
                default:
                    RoundedRectangle(cornerRadius: 14)
                        .fill(KonomiTheme.inkSecondary.opacity(0.15))
                        .overlay(Image(systemName: item.mediaType.icon).foregroundStyle(KonomiTheme.inkSecondary))
                }
            }
            ...
            RatingView(selectedScore: viewModel.ratingDrafts[item.id] ?? nil) { score in
    ...
    private var completionView: some View {
        if let result = viewModel.completionResult {
            VStack(spacing: 18) {
                ...
                Text(
                    "Added \(countLabel(result.addedMovies, singular: "movie", plural: "movies")) "
                    + "and \(countLabel(result.addedTVShows, singular: "TV show", plural: "TV shows"))"
                )
                    .font(.headline)
                    .foregroundStyle(KonomiTheme.ink)
                ...
                VStack(spacing: 12) {
                    ...
                    if totalRatedLibraryCount(result: result) >= TasteAnalysisService.minimumRatedItems {
                        Button {
                            navigationState.navigateToTaste(.profile)
                            dismiss()
                        } label: {
    ...
    private var detailOverlay: some View {
        ZStack {
            ...
            if let detail = viewModel.detailState {
                VStack(alignment: .leading, spacing: 14) {
                    HStack(alignment: .top, spacing: 14) {
                        AsyncImage(url: URL(string: detail.item.coverURLString ?? "")) { phase in
                            ...
                            default:
                                RoundedRectangle(cornerRadius: 12)
                                    .fill(KonomiTheme.inkSecondary.opacity(0.18))
                                    .overlay(Image(systemName: detail.item.mediaType.icon).foregroundStyle(KonomiTheme.inkSecondary))
                            }
                        }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            if settings.first?.showPublicScores ?? true, let score = detail.item.publicScore {
                                Text("Public: \(Formatters.publicScore(score))")
                                    .font(.caption)
                                    .foregroundStyle(KonomiTheme.inkSecondary.opacity(0.85))
                            }
                ...
                .accessibilityLabel("Details for \(detail.item.title)")
            }
        }
    }
                    ...
                    AsyncImage(url: URL(string: item.coverURLString ?? "")) { phase in
                        ...
                        default:
                            RoundedRectangle(cornerRadius: KonomiTheme.coverRadius)
                                .fill(KonomiTheme.inkSecondary.opacity(0.14))
                                .overlay(Image(systemName: item.mediaType.icon).foregroundStyle(KonomiTheme.inkSecondary))
                        }
                    }
                    .frame(height: 168)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            RoundedRectangle(cornerRadius: 4)
                ...
                .frame(height: 12)
            RoundedRectangle(cornerRadius: 4)
                .fill(KonomiTheme.inkSecondary.opacity(0.1))
